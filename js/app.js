...
const lang=$('#lang');lang?.addEventListener('click',()=>{const en=root.lang==='en';root.lang=en?'fa':'en';root.dir=en?'rtl':'ltr';lang.textContent=en?'EN':'FA';localStorage.setItem('uhm-lang',root.lang)}); if(localStorage.getItem('uhm-lang')==='en'){root.lang='en';root.dir='ltr';if(lang)lang.textContent='FA'}
...
$$('.copy').forEach(b=>b.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(b.closest('.path').dataset.copy);b.textContent='کپی شد ✓';setTimeout(()=>b.textContent='کپی مسیر',1600)}catch{b.textContent='انتخاب کنید'}}));
